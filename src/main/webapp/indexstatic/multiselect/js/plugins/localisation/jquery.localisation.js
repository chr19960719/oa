...
$.localise = function(packages, settings, loadBase, path, timeout) {
	...
	var localiseOne = function(package1, lang) {
		if (settings.loadBase) {
			$.getScript(paths[0] + package1 + '.js');
		}
		if (lang.length >= 2) {
			$.getScript(paths[1] + package1 + '-' + lang.substring(0, 2) + '.js');
		}
		if (lang.length >= 5) {
			$.getScript(paths[1] + package1 + '-' + lang.substring(0, 5) + '.js');
		}
	};
